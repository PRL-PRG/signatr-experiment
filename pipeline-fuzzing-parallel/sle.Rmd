...
overview_table(
  ...
  r("Num functions baseline signatrue", filter(signatures_summary, shared > 0 | only_baseline > 0)),
  ...
  r("Num functions baseline to corpus signature ratio", ratio(count(baseline_types, fun_name), all_traces_stats)),
  r("Avg new signatr signature", mean(filter(signatures_summary, only_fuzz > 0)$only_fuzz)),
  r("Shared signatues", sum(signatures_summary$shared)),
...
ggplot(aes(x = reorder(fun_name, all, sum), color = name)) +
    geom_linerange(data = filter(D, name == "baseline"), aes(ymin = 0, ymax = -value), size = .1) +
    geom_linerange(data = filter(D, name == "fuzz"), aes(ymin = 0, ymax = value), size = .1) +
    scale_y_continuous(
    ...
    annotate("text", x = 500, y = 0.5, label = "Fuzzing", size = 4) +
    annotate("text", x = 500, y = -0.5, label = "Tracing", size = 4)
...
ggsave("uf-call-signatures.pdf", width = 6)
```
